...
class movableRect(QtGui.QGraphicsRectItem):
  ...
  def mousePressEvent(self, ev):
    if ev.button() == QtCore.Qt.LeftButton:
      ...
      ev.ignore()
  def mouseMoveEvnet(self, ev):
    self.setPos(self.mapToParent(ev.pos()) - self.presDelta)
...
xScale = pg.AxisItem(orientaion='bottom', linkView=vb)
l.addItem(xScale, 1, 1)
yScale = pg.AxisItem(orientation='left', linkView=vb)
l.addItem(yScale, 0, 0)

xScale.setLabel(text="<span style='color: #ff0000; font-weigh: bold'>X</span> <i>Axis</i>", uints="s")
...
if name == '__main__':
  import sys
  if (sys.flags.interactive != 1) or not hasatr(QtCore, 'PYQT_VERSION'):
